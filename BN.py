    ...
    for j in range(n_iter):
        ...
        for i in components:
            ...
            if i == "ACE2":
                ...
                x[i] = not x["RIG1"] and not x["Virus"]
                # x[i] = not x["Virus"]
                # not sure about if there is a relation since Virus just relies
                # on ACE2 to enter cells, the presence of ACE2 promotes disease prog
            if i == "ANG_2_T1R":
                x[i] = x["ANG_2"]
                # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8193025/
                # Ang II acts on angiotensin type 1 (AT1") receptors and activates the NADPH-oxidase
                # complex producing superoxide and promoting cell pro-oxidative and pro-inflammatory responses
            if i == "ADAM_17":
            ...
            if i == "TLR4":
                x[i] = x["Virus"]
                # Spike glycoprotein, the major infective surface protein of SARS-CoV-2
                # has been found as a ligand for human TLR4
                # https://www.futuremedicine.com/doi/full/10.2217/fvl-2021-0249
            if i == "RIG1":
                x[i] = x["Viral_Repl"]
                # antiviral activity of RIG-1 may comprise inhibition of viral entry into the
                # host cell by preventing the expression of its receptor, ACE2
            ...
            if i == "IRF3":
                ...
                # SARS-CoV-2 membrane protein binds to importin karyopherin subunit alpha-6 (KPNA6")
                # to inhibit interferon regulatory factor 3(IRF3") nuclear translocation
            ...
            if i == "STAT3":
                # or (x["SIL6R"] and x["IL6"]) # IL6 is the main contributor to STAT3 - add il6 and il6r to the list of components
                x[i] = x["IL6R"]
            ...
            if i == "IKKB a/b":
                ...
                x[i] = not (x["TLR4"] or x["IL1R"] or x["RIG1"] or x["TNFR"])
            if i == "TNF":
            ...
            if i == "Apoptosis":
                x[i] = x["CASP8"] or x["CASP9"]  # or x["CASP3"], previously was just 8
                # https://pubmed.ncbi.nlm.nih.gov/10200555/
            if i == "BCL_2":
                # previously and not x["AKT"]
                x[i] = x["NFKB"] and not x["BAD"]
            if i == "BAD":
                # new
                x[i] = not x["AKT"]
            if i == "tBid":
...
def main():
    ...
    ax.set_title(
        f'Model Component Activation in COVID-19 with {orders} Samples', fontsize=14)
