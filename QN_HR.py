...
def generate_boxplots_ggplot(filename = 'results.csv'):
    ...
    """
    import pandas as pd
    from plotnine import ggplot, aes, geom_boxplot, geom_jitter, labs, theme_bw, theme, element_text
...
def generate_APO_test(filename = 'results.csv'):
    """
    Generates boxplots for the simulation results using ggplot using filtered data.
    """

    import pandas as pd
    from plotnine import ggplot, aes, geom_boxplot, position_dodge, theme_linedraw, theme_light, labs

    df = pd.read_csv(filename)
    default_names = df.iloc[0, 0:4].values
    
    df = df[(df["Component"] == "APO") & (df["IFN_Scenario"] == 0)]
    names = [""] * len(df)
    ...
    variety = [f"{row[4]}_{row[5]}" for row in df.itertuples(index=False, name=None)]

    # Ensure the lengths of names and variety match Apoptosis
    if len(df) != len(names) != len(variety):
    ...
    # Create the plot
    p = (ggplot(data, aes(x='names', y='Value', fill='variety'))
         ...
         + theme_linedraw()
         + theme_light()
         + labs(x="Variety", y="Apoptosis", title="Apoptosis Activation in COVID-19, IFN Scenario 0"))
...
if __name__ == '__main__':
    # main()
    # generate_boxplots("results.csv")
    # generate_boxplots_ggplot("results.csv")
    generate_APO_test("results.csv")
    print("Complete")